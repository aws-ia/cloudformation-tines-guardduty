...
This Quick Start requires a registered {partner-product-short-name} account with any plan.

To purchase a paid plan through the AWS Marketplace, do the following steps:
...
[cols=2*]
|===
^|https://fwd.aws/E4m9w[Deploy {partner-product-short-name} GuardDuty integration on AWS^]
...
. On the *Specify stack details* page, change the stack name if needed. Review the parameters for the template. Provide values for the Tines webhook address that requires input. For details on each parameter, see the link:#_parameter_reference[Parameter reference] section of this guide. When you finish reviewing and customizing the parameters, choose *Next*.
